...
function initArticlePage() {
  ...
  document.title   = `${article.title} — TD بالعربي`;
  ...
  if (sourceEl) {
    ...
    sourceEl.textContent = src
      ? `المصدر: ${escapeHtml(src)} | أُعيدت كتابته بواسطة TD بالعربي`
      : 'أُعيدت كتابته بواسطة TD بالعربي';
  }
...
function initOneSignal() {
  ...
  const bannerKey = 'tdn_notif_banner_shown';
  const consentKey = 'tdn_notif_consent';

  // Check if already subscribed or banner was dismissed
  if (localStorage.getItem(consentKey) === 'dismissed') return;
  if (localStorage.getItem(bannerKey)) return;

  // Show notification permission banner after 8s delay
  setTimeout(() => {
    if (localStorage.getItem(consentKey) === 'dismissed') return;
...
function showNotificationBanner() {
  ...
  localStorage.setItem('tdn_notif_banner_shown', '1');
  ...
  banner.innerHTML = `
    <p>🔔 هل تريد تلقي إشعارات فورية عند نشر أخبار جديدة؟</p>
    <div class="notif-banner-actions">
      <button class="notif-banner-btn notif-btn-allow" id="notifAllow">تفعيل الإشعارات</button>
      <button class="notif-banner-btn notif-btn-deny" id="notifDeny">ليس الآن</button>
    </div>
  `;
  ...
  document.getElementById('notifAllow')?.addEventListener('click', () => {
    if (window.OneSignal) {
      window.OneSignal.Notifications.requestPermission().then((accepted) => {
        if (accepted) {
          localStorage.setItem('tdn_notif_consent', 'granted');
      ...
      showToast('خدمة الإشعارات غير متوفرة حالياً');
      banner.remove();
    }
  });

  document.getElementById('notifDeny')?.addEventListener('click', () => {
    localStorage.setItem('tdn_notif_consent', 'dismissed');
    banner.remove();
  });
}

/* =========================================================
   13. SUBSCRIPTION POPUP
   ========================================================= */
function initSubscriptionPopup() {
  const overlay = document.getElementById('subscribeOverlay');
  if (!overlay) return;

  const POPUP_KEY = 'tdn_popup_dismissed';
  ...
  // Show popup after 30s delay
  const showTimer = setTimeout(() => {
    if (localStorage.getItem(POPUP_KEY)) {
      const dt = parseInt(localStorage.getItem(POPUP_KEY), 10);
      if (Date.now() - dt < 7 * 24 * 60 * 60 * 1000) return;
    }
    overlay.hidden = false;
    document.body.style.overflow = 'hidden';
  }, 30000);
  ...
  const skipBtn = document.getElementById('subscribeSkip');
  ...
  form?.addEventListener('submit', (e) => {
    ...
    if (input && input.value) {
      // Store email in localStorage (prototype)
      localStorage.setItem('tdn_subscriber_email', input.value);
      showToast('شكراً! تم الاشتراك بنجاح ✓');
      dismissPopup();
    }
  });
}
...
/* =========================================================
   14. AD SLOT ENHANCEMENT
   ========================================================= */
function initAdSlots() {
  // In a real implementation, this would load ads from an ad network.
  // For the prototype, we show recognizable ad placeholders.
  const slots = document.querySelectorAll('.ad-container');
  slots.forEach(slot => {
    const location = slot.dataset.adLocation;
    const sizes = {
      'hero': '728×90 — banner',
      'infeed': '300×250 — مربع',
      'footer': '728×90 — banner',
      'article-top': '728×90 — banner',
      'article-bottom': '300×250 — مربع'
    };
    const label = sizes[location] || 'إعلان';
    slot.textContent = label;
    slot.style.cssText += 'display:flex;align-items:center;justify-content:center;color:var(--muted);font-size:.9rem;font-weight:600;';
  });
  ...
  if ('IntersectionObserver' in window) {
    const adObserver = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          const loc = entry.target.dataset.adLocation;
          try {
            if (window.goatcounter) {
              goatcounter.count({ path: '/ad-impression/' + (loc || 'unknown') });
            }
          } catch {}
    ...
    document.querySelectorAll('.ad-container').forEach(el => adObserver.observe(el));
  }
}

/* =========================================================
   15. TELEGRAM CTA TRACKING
   ========================================================= */
function initTelegramCTA() {
  // Track clicks on all Telegram CTAs
  document.querySelectorAll('a[href*="t.me"]').forEach(link => {
    link.addEventListener('click', () => {
      try {
        if (window.goatcounter) {
          goatcounter.count({ path: '/telegram-cta-click' });
        }
      } catch {}
    });
  });
...
/* =========================================================
   17. UPDATE DOMContentLoaded INIT
   ========================================================= */
// Patch: initialize new features after articles load
(function patchInit() {
  ...
  document.addEventListener = function(type, fn, opts) {
    if (type === 'DOMContentLoaded') {
      const wrapped = function(e) {
        fn(e);
        // Initialize new features after DOM ready
        setTimeout(() => {
          initServiceWorker();
          initOneSignal();
          ...
          notice.textContent = '🧪 TD بالعربي — بيئة تجريبية للاختبار';
          document.body.prepend(notice);
        }, 100);
      };
      return origAddEventListener.call(document, type, wrapped, opts);
    }
    return origAddEventListener.call(document, type, fn, opts);
  };
})();
