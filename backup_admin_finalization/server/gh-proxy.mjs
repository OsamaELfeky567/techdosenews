 ...
 *
 * Authenticates via GitHub App (preferred) or PAT (legacy fallback).
 *
 * GitHub App usage:
...
createServer(async (req, res) => {
  ...
  req.on('end', async () => {
    try {
      ...
      if (!token) {
        writeJson(401, { error: 'No authentication configured. Set GITHUB_APP_ID + GITHUB_APP_PRIVATE_KEY + GITHUB_INSTALLATION_ID or GH_TOKEN.' });
        return;
      }
  ...
  const authMethod = GITHUB_APP_ID ? 'GitHub App' : (GH_TOKEN ? 'PAT (legacy)' : 'NO AUTH');
